...
const trans = 'cbdefghijklnrtuv'
// convert number to 2 digit hex = 255 --> ff
function toHex (n) { return ('0' + n.toString(16)).substr(-2) }
...
function decode (src) {
  ...
  for (var i = 0; i < src.length; i++) {
    ...
    if (p1 === -1) { return null }
    b = (p1 === -1) ? 0 : p1
    if ((flag = !flag)) { dst = b } else { hex += toHex(dst << 4 | b) }
  }
  return hex
}
...
module.exports = { decode: decode, decodeInt: decodeInt }
